...
class CountdownWidget extends StatefulWidget {
  const CountdownWidget(
      {@required this.width, Key key, this.duration, this.triviaStatus})
      : assert(width != null),
        super(key: key);
  ...
  @override
  CountdownRebuilder createState() {
    return CountdownRebuilder();
  }
}

class CountdownRebuilder extends State<CountdownWidget>
  ...
  void initState() {
    ...
    animation = Tween<double>(begin: widget.width, end: 0).animate(controller)
      ..addListener(() {
        ...
        if (widget.triviaStatus.isAnswerChosen) {
          controller.stop();
        }
      });
